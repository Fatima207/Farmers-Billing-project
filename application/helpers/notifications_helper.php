...
if (!function_exists('send_notification_emails')) {
    ...
    function send_notification_emails($notification_id, $email_notify_to, $extra_data = array()) {
        ...
        /* cases for different messages 
            1) if event added by end user then message will be event assign to you
            2) if event updated more than one and added by end user then message will be internal user updated end user event
            3) if event updated again which is not added by end user then message will be event updated by
        */

        $eve_row = $ci->db->select('client_id, update_count, created_by, share_with')->get_where('events', array('id' => $notification->event_id))->row();

        // pr($eve_row);// exit;

        if($eve_row->client_id == 0 && $eve_row->update_count == 0) {
            $msg = 'Added event';
        } else if($eve_row->client_id == 0 && $eve_row->update_count > 0) {
            $msg = 'Updated event';
        } else if($eve_row->client_id != 0 && $eve_row->update_count > 0) {
            $msg = 'Shared a visitor visit';
        } else {
            $msg = 'Added event';
        }

        $parser_data["EVENT_TITLE"] = $notification->user_name . " " . $msg ." ". $notification->to_user_name;
        $subject = $ci->parser->parse_string($email_template->subject, $parser_data, TRUE);

        $message_id_hash = md5(rand(1,100000) . $notification->user_name . time());
        //"[EVENT_TITLE]" => "<b>" . $notification->user_name . "</b> " . sprintf(lang("notification_" . $notification->event), $notification->to_user_name),
        $message = $ci->common->replace_keywords(array(
            "[APP_TITLE]" => get_setting_from_db("app_title"),
            "[COMPANY_NAME]" => get_setting_from_db("company_name"),
            "[EVENT_TITLE]" => "<b>" . $notification->user_name . "</b> " . $msg,
            "[NOTIFICATION_URL]" => "<a href='".$url."'>Event Detail</a>",
            "[EVENT_DETAILS]" => $ci->load->view("notifications/notification_description", $view_data, true),
            ),
        $email_template->message);
        $headers = array(
            "Message-ID" => $message_id_hash
        );
        //$subject = "Submit your working hours.";
        ...
        if ($email_notify_to) {
            ...
            foreach ($email_notify_to_array as $email_address) {
                ...
                //$ci->common->send_email($email_template->subject, $email_template->message, $email, $headers);
                //$ci->common->send_email($subject, $message, $email_address, $headers);
                $to = get_user_id($email_address);// exit;
                $url = 'events/index/'.$id = encode_id($notification->event_id, "event_id");
                // $from = get_user_id($eve_row->created_by);
                // name is confusing but used for common functionality that is notify to user
                $ci->notify->notifyRejoinUser($eve_row->created_by, $to, $type='event', $request_month='', $msg, $url, $description='');
            }
        }
