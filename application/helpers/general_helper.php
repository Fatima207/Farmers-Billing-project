...
if (!function_exists('get_setting_from_db')) {

    function get_setting_from_db($key = "") {
        $ci = get_instance();
        $query = $ci->db->select('setting_value')->get_where('other_settings', array('setting_name' => $key, 'type' => 'app', 'deleted' => 0));
        if($query->num_rows()==0) {
            return '';
        }
        $queryData = $query->row();
        // echo $ci->db->last_query();
        return $queryData->setting_value;
        //return $ci->config->item($key);
    }
...
if (!function_exists('send_app_mail')) {
    ...
    function send_app_mail($to, $subject, $message, $optoins = array()) {
        ...
        // echo "HH".get_setting_from_db("email_protocol");
        //check mail sending method from settings
        if (get_setting_from_db("email_protocol") === "smtp") {
            // echo 33;
            $email_config["protocol"] = "smtp";
            $email_config["smtp_host"] = get_setting_from_db("email_smtp_host");
            $email_config["smtp_port"] = get_setting_from_db("email_smtp_port");
            $email_config["smtp_user"] = get_setting_from_db("email_smtp_user");
            $email_config["smtp_pass"] = get_setting_from_db("email_smtp_pass");
            $email_config["smtp_crypto"] = get_setting_from_db("email_smtp_security_type");
        ...
        // echo 44;
        // echo get_setting_from_db("email_sent_from_address");
        // exit;
        $ci = get_instance();
        $ci->load->library('email', $email_config);
        $ci->email->clear(true); //clear previous message and attachment
        $ci->email->set_newline("\r\n");
        $ci->email->from(get_setting_from_db("email_sent_from_address"), get_setting_from_db("email_sent_from_name"));
        $ci->email->to($to);
...
if (!function_exists('get_team_member_profile_link')) {
    ...
    function get_team_member_profile_link($id = 0, $name = "", $attributes = array()) {
        ...
        if ($ci->user->info->user_from == "support") {
            return anchor("profile/visitor_profile/" . $id, $name, $attributes);
        } else {
...
if (!function_exists('get_team_members_and_teams_select2_data_list')) {
    ...
    function get_team_members_and_teams_select2_data_list() {
        ...
        $team_members = $ci->Users_model->get_all_where(array("user_from" => "support"))->result();
        $members_and_teams_dropdown = array();
        ...
        $team = $ci->Team_model->get_all_where(array("status" => 1))->result();
        foreach ($team as $team) {
            $members_and_teams_dropdown[] = array("type" => "team", "id" => "team:" . $team->ID, "text" => $team->name);
        }
...
if (!function_exists('log_notification')) {
    ...
    function log_notification($event, $options = array(), $user_id = 0) {
        ...
        } else if (isset($ci->user->info)) {
            $req .= "&user_id=" . $ci->user->info->ID;
        }
...
if (!function_exists('get_visitor_profile_link')) {

    function get_visitor_profile_link($id = 0, $name = "", $attributes = array()) {
        $ci = get_instance();
        if ($ci->user->info->user_from === "support") {
            return anchor("profile/visitor_profile/" . $id, $name, $attributes);
        } else {
