            ...
            <ul v-if="menuState.registration.open" class="nav flex-column pl-3">
              ...
              <li class="nav-item">
                ...
                <ul v-if="menuState.registration.subMenu.farmers" class="nav flex-column pl-3">
                  ...
                  <li class="nav-item">
                    <a :href="farmersListsrc" class="nav-link">View All</a>
                  </li>
                </ul>
              </li>
              <!-- Level 2 retailers -->
            ...
            <ul v-if="menuState.billing.open" class="nav flex-column pl-3">
              ...
              <li class="nav-item">
                ...
                <ul v-if="menuState.billing.subMenu.farmers" class="nav flex-column pl-3">
                  ...
                  <li class="nav-item">
                    <a :href="BillingfarmersListsrc" class="nav-link">View All</a>
                  </li>
                </ul>
              </li>
              ...
              <div class="form-group">
                <div class="row" style='justify-content: space-between;'>
                  ...
                  <div class="col-md-2">
                    ...
                    <select id="farmers" name="farmer" v-model="form.farmer" @change="fetchFarmerDetails" required>
                      <option v-for="farmers in farmers" :key="farmers.id" :value="farmers.id">
                        {{ farmers.name }}
                      </option>
                    </select>
                  </div>
                        ...
                        <div class="card-body table-responsive p-0"
                          ...
                          <table class="table table-head-fixed text-nowrap" id="recordListing">
                            ...
                            <tbody>
                              ...
                              <!-- If a farmers is selected, display their details -->
                              <tr v-if="selectedFarmerDetails">
                                <td>{{ selectedFarmerDetails.name }}</td>
                                <td>{{ selectedFarmerDetails.code }}</td>
                                <td>{{ selectedFarmerDetails.address }}</td>
                                <td>{{ selectedFarmerDetails.contact_number }}</td>
                              </tr>
                              <tr v-else>
                                <td>{{ form.temp }}</td>
                                <td>{{ form.code }}</td>
                  ...
                  <div style="display: flex; align-items: center;">
                    ...
                    <div style="margin-left: 450px; display: flex; gap: 10px;">
                      ...
                      <button type="submit" class="btn btn-dark btn-sm" id="editfarmer" style="width: 60px;">
                        Save
                      </button>
                    </div>
                  </div>
...
export default {
    ...
    computed: {
        selectedFarmerDetails() {
            // Find the farmers object that matches the selected farmers's ID
            return this.frms.find(farmer => farmer.id === this.farmers);
        },
    ...
    methods: {
        ...
        fetchBillingDetails(id) {
            fetch(`http://localhost/dairy/index.php/Home/edit_BillingFarmer/${id}`)
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        if (data.extraFields) {
                            this.form = {
                                ...this.form,
                                ...data.extraFields, // Add top-level fields (e.g., billing_id, farmer, etc.)
                            };
                        }
        ...
        fetchFarmerDetails() {
            if (!this.form.farmer) {
                this.selectedFarmerDetails = null; // Clear details if no farmer selected
                return;
            }
            console.log("Form Data:", this.form);
            console.log("Selected farmer Details:", this.selectedFarmerDetails);
            axios
                .get(`http://localhost/dairy/index.php/Home/get_FarmerDetails/${this.form.farmer}`)
                .then(response => {
                    console.log("Form Data:", this.form.farmer);
                    console.log("Selected farmer Details:", this.selectedFarmerDetails);
                    this.selectedFarmerDetails = response.data; // Store selected farmer's details
                })
                .catch(error => {
        ...
        submitEditForm() {
            ...
            axios
                .post("http://localhost/dairy/index.php/Home/update_Billingfarmer", formData, {
                    headers: {
        ...
        getfarmers() {
            axios.get('http://localhost/dairy/index.php/Home/get_farmers')
                .then((response) => {
                    ...
                    this.farmers = response.data;  // Assign the fetched data to the 'farmers' array
                })
                .catch(error => {
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        fields: [],
        // paymentStatus: "",
        frms: [],              // This will store the list of farmerss from the backend
        billingList: [], // Array to store the billing list
        comp: [],
        product_id: [],
        prod: [
        ...
        // selectedFarmerDetails: [], // Store details of selected farmer
        selectedProductIds: [],
        prefilledProducts: [1, 2], // Example prefilled product IDs
  ...
  Reports: {
    ...
    computed: {
      selectedFarmerDetails() {
        // Find the farmers object that matches the selected farmers's ID
        return this.farmers.find(farmers => farmers.id === this.form.farmer);
      },
      totalSelectedProducts() {
        const allProducts = [...new Set([...this.prefilledProducts, ...this.selectedProducts])];
        console.log("All Products:", allProducts);
                        ...
                        <div class="form-group">
                            <div class="row" style='justify-content: space-between;'>
                                ...
                                <div class="col-md-2">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div class="input-group-append">
                                            <div class="input-group">
                                                <!-- Date Picker Input -->
                                                <input type="text" class="form-control" ref="datepicker"
                                                    name="created_at" v-model="form.created_at"
                                ...
                                <div class="col-md-2">
                                    ...
                                    <select id="Farmers" name="farmer" v-model="form.farmer"
                                        @change="fetchFarmerDetails" required>
                                        <option v-for="farmer in frms" :key="farmer.id" :value="farmer.id">
                                            {{ farmer.name }}
                                        </option>
                                    </select>
                                ...
                                <div class="row" style="padding-left:10px;">
                                    <div class="col-12">
                                        <div class="card" style="display:flex;flex-direction:row;">
                                            ...
                                            <!-- previous -->
                                            <div class="card-body  table-responsive p-0"
                                                style="flex-shrink:2;border:1px solid black;">
                                                <label style='padding-right:100px;'>Previous Dues Amount</label>
                                                <span>Rs</span>
                                            </div>
                                        </div>
                                ...
                                <br>
                                <div class="row"
                                    style="padding-left: 5px; padding-right: 5px; margin-left: 0; margin-right: 0;">
                                    ...
                                    <div class="col-md-2"
                                        ...
                                        <div class="form-group">
                                            <label>Select Products</label>
                                            <select id="Products" v-model="selectedProductIds"
                                                @change="updateSelectedProducts" multiple class="form-control">
                                                <option v-for="product in prod" :key="product.id" :value="product.id"
                                                    @click="onProductSelect(product.id)">
                                                    {{ product.name }}
                                                </option>
                                    ...
                                    <div class="col-md-7"
                                        ...
                                        <div class="card card-row card-primary mx-2 text-sm font-small"
                                            ...
                                            <div class="card-body">
                                                <div v-for="product in fields" :key="product.id" class="mb-4">
                                                    <h5>{{ product.name }}</h5>
                                                    <button @click="addField(product)"
                                                        class="btn btn-success">+</button>
                                                    <label class="mx-5">Total Amount: Rs {{
                                                        calculateTotalAmount(product) }}</label>
                                                    <label class="ml-5">Total Quantity: {{
                                                        calculateTotalQuantity(product) }}</label>

                                                    <!-- Fields for Quantity, Unit, and Price -->
                                                    <div v-for="(field, index) in product.fields || []" :key="index"
                                                        ...
                                                        <div class="d-flex flex-column"
                                                            ...
                                                            <label>Qty</label>
                                                            <input type="number" name="qty[]" v-model="field.qty"
                                                        ...
                                                        <div class="d-flex flex-column"
                                                            style="flex: 1; min-width: 120px;">
                                                            <label>Unit</label>
                                                            <select v-model="field.unit" name="unit[]"
                                                        ...
                                                        <div class="d-flex flex-column"
                                                            style="flex: 1; min-width: 120px;">
                                                            <label>Price</label>
                                                            <input type="number" name="price[]" v-model="field.price"
                                                        ...
                                                        <div class="d-flex flex-column"
                                                            style="flex: 1; min-width: 120px;">
                                                            <label>Amount: Rs {{ calculateAmount(field.price, field.qty)
                                                                }}</label>
                                                            <button @click="removeField(product, index)"
                                    ...
                                    <!-- Third Section: Final totals and payment details -->
                                    <div class="col-md-3"
                                        style="display: flex; flex-direction: column; padding-left: 5px;">
                                        <div class="card card-row card-primary font-small text-sm"
                                            style="border: 1px solid black; border-radius: 8px; flex: 1;">
                                            <div class="card-body">
                                                <p class="text-center">Final total: Rs {{ totalAmount }}</p>
                                                ...
                                                <!-- Commission input -->
                                                <label for="commission">Commission in %</label>
                                                <input type="number" v-model.number="form.commission" placeholder="0"
                                                    class="w-25" name="commission" @input="updateCommission" /><br>

                                                <label class="font-small">Commission amount: Rs {{ commissionAmount
                                                    }}</label>
                                                ...
                                                <div class="text-center">
                                                    ...
                                                    <span>
                                                        <label for="arhat">Arhat coolie :</label>
                                                        <label for="britty" class="pl-5">Britty</label><br>
                                                        <input type="number" v-model.number="form.arhat_coolie"
                                                            name="arhat_coolie" placeholder="0" class="w-25 mx-3" />
                                                        <input type="number" v-model.number="form.britty"
                                                            placeholder="0" name="britty" class="w-25" /><br>
                                                    </span>
                                                    <span>
                                                        <label for="dan">Dan :</label>
                                                        <label for="jeep_fair" class="pl-5">Jeep fair :</label><br>
                                                        <input type="number" v-model.number="form.dan" placeholder="0"
                                                            name="dan" class="w-25 mx-3" />
                                                        <input type="number" v-model.number="form.jeep_fair"
                                                            placeholder="0" name="jeep_fair" class="w-25" /><br>
                                                    </span>
                                                    <span>
                                                        <label for="rail_coolie">Rail Coolie :</label>
                                                        <label for="ice_Leaf" class="pl-5">Ice leaf :</label><br>
                                                        <input type="number" v-model.number="form.rail_coolie"
                                                            placeholder="0" name="rail_coolie" class="w-25 mx-3" />
                                                        <input type="number" v-model.number="form.ice_leaf"
                                                            placeholder="0" name="ice_leaf" class="w-25" /><br>
                                                    </span>

                                                    <span>
                                                        <label for="unio_n">unio_n :</label>
                                                        <label for="misc_exp" class="pl-5">Misc Exp. :</label><br>
                                                        <input type="number" v-model.number="form.unio_n"
                                                            placeholder="0" name="unio_n" class="w-25 mx-3" />
                                                        <input type="number" v-model.number="form.misc_exp"
                                                            placeholder="0" name="misc_exp" class="w-25" /><br>
                                                    </span>

                                                    <span>
                                                        <label for="market_exp">Market Exp. :</label><br>
                                                        <input type="number" v-model.number="form.market_exp"
                                                            placeholder="0" name="market_exp" class="w-25 mx-3" />
                                                    </span>
                                                </div>
                                                <!-- Total charged amount and Grand Total -->
                                                <label for="totalCharged" style="font-size: 20px;">Total charged amount:
                                                    Rs {{ totalCharged
                                                    }}</label><br>
                                                <div class="text-center">
                                                    <button>
                                                        ...
                                                        <!-- Add a hidden input to send grand_total with the form -->
                                                        <label for="roundedOffAmount">Rounded off amount: Rs {{
                                                            roundedTotal }}</label><br>
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div class="d-flex align-items-center justify-content-start gap-3 mt-3"
                                    ...
                                    <label for="qty" class="font-small text-sm px-5">Total Quantity: {{ totalQuantity
                                        }}</label>
                                    <label for="qty" class="font-small text-sm px-3">Total Products: {{
                                        totalSelectedProducts }}</label>
                                ...
                                <div style="display: flex; align-items: center;">
                                    ...
                                    <!-- Payment Modes -->
                                    <div>
                                        <label style="display: block; font-weight: bold; margin-bottom: 5px;">Payment
                                            Modes</label>
                                        <div style="display: flex; align-items: center; gap: 10px;">
                                            <!-- Cash Payment Field -->
                                            <div>
                                            ...
                                            <div>
                                                <label for="cheque" style="display: block;">Cheque</label>
                                                <input type="number" v-model.number="form.cheque" name="cheque"
                                                    id="cheque" placeholder="Enter Cheque Amount"
                                                    style="width: 100px;" />
                                            </div>
                                            ...
                                            <div>
                                                <label for="online" style="display: block;">Online</label>
                                                <input type="number" v-model.number="form.online" name="online"
                                                    id="online" placeholder="Enter Online Payment"
                                                    style="width: 100px;" />
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <!-- Total Dues -->
                                    <div style="margin-left: 20px;">
                                        <label for="totalDues" style="display: block;">Total Dues : {{ totalDues
                                            }}</label>
                                        <input type="hidden" v-model="form.total_dues" name="total_dues" />
                                    </div>
                                    ...
                                    <!-- Buttons -->
                                    <div style="margin-left: 450px; display: flex; gap: 10px;">
                                        <button type="button" class="btn btn-primary btn-sm" @click="printPage"
                                            style="width: 100px;">
                                            Print & Save
                                        </button>
                                        <button type="submit" class="btn btn-dark btn-sm" id="editfarmer"
                                            style="width: 60px;">
                                            Save
                                        </button>
                                    </div>
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        fields: [],
        // paymentStatus: "",
        farmers: [],              // This will store the list of farmerss from the backend
        billingList: [], // Array to store the billing list
        comp: [],
        product_id: [],
        prod: [
    ...
    computed: {
      selectedFarmerDetails() {
        // Find the farmers object that matches the selected farmers's ID
        return this.frms.find(farmer => farmer.id === this.form.farmer);
      },
      totalSelectedProducts() {
        const allProducts = [...new Set([...this.prefilledProducts, ...this.selectedProducts])];
        console.log("All Products:", allProducts);
    ...
    updateSelectedProducts() {
      ...
      getFarmers() {
        axios.get('http://localhost/dairy/index.php/Home/get_farmers')
          .then((response) => {
            console.log(response.data); // This will log the data fetched from the backend
            this.frms = response.data;  // Assign the fetched data to the 'farmers' array
          })
          .catch(error => {
            console.error(error); // Log errors if the request fails
          });
